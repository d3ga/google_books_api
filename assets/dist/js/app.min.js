...
class GoogleBooksApi{constructor(s){this.defaults={apiUrl:"https://www.googleapis.com/books/v1",apiKey:"AIzaSyDmbXk-aptT2LrpnW5cHzgYnT7noS7IjeQ"},this.elements={form:$("#form"),input:$("#search-input")},this.options=Object.assign(this.defaults,s),this.registerEvents()}callApi(s,t){const e=`${this.options.apiUrl}/volumes`,o={q:s};$.getJSON(e,o).done(s=>{console.log(s),this.showResults(s),t(s)}).fail(s=>{t(null)})}registerEvents(){this.elements.form.on("submit",s=>{s.preventDefault(),this.callApi(this.elements.input.val().trim(),s=>{console.log(s)})})}showResults(s){s.items.forEach(s=>{this.elements.form.after(`\n             <div class="card">\n                 <img class="card-img-left" src="${s.volumeInfo.imageLinks.thumbnail}" alt="Card image cap">\n                <div class="card-body">\n                  <h3 class="card-title book-title">${s.volumeInfo.title}</h3>\n                  <h4 class="card-title author-name">${s.volumeInfo.authors}</h4>\n                  <p class="card-text">${s.volumeInfo.description}</p>\n               </div>\n            </div>\n        `)})}}const bookSearch=new GoogleBooksApi({});